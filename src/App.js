...
function WatchedSummary({ watched }) {
  ...
  const avgImdbRating = average(watched.map((movie) => movie.imdbRating)); // Extracts all imdbRating values and computes average
  const avgUserRating = average(watched.map((movie) => movie.userRating)); // Same for userRating
  const avgRuntime = average(watched.map((movie) => movie.runtime)); // Sme for runtime
  return (
